...
module.exports = function (options, done) {
  ...
  server.getConnection(function (c) {
    ...
    async.waterfall([
      ...
      exports.exec.bind(null, 'git rev-parse HEAD'),
      ...
      // Get changed files
      function (next) {
        exports.exec('git diff --name-status ' + remoteCommit + ' ' + localCommit, next);
      },
      function (stdout, stderr, next) {
        ...
        if(!remoteCommit) {
          ...
          exports.exec('git ls-tree -r ' + localCommit + ' --name-only', {
            maxBuffer: 1024*1000
          }, function (err, stdout) {
            if(err) {
              return next(err);
      ...
      function (next) {
        ...
        async.eachLimit(diff.M, options.eachLimit || 10, function (file, next) {
          var remote = path.join(options.remotePath, file),
              local = path.join(process.cwd(), file);
      ...
      function (next) {
        ...
        async.eachLimit(diff.D, options.eachLimit || 10, function (file, next) {
          logger.debug(pkg.name, 'Deleting ' + file);
          c.delete(path.join(process.cwd(), file), path.join(options.remotePath, file), next);
        }, next);
      },
